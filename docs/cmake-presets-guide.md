...
| `configurePresets` | configure 阶段的 preset 数组 | 13 个 hidden + 44 个 leaf |
| `buildPresets` | build 阶段的 preset 数组 | 56 个 |
| `testPresets` | ctest 的 preset 数组 | 1 个 hidden + 56 个 |
...
40+ 个 preset 就要重复 40+ 次相同的字段。**`hidden` + `inherits` 让你把公共字段抽出来当
积木**。
...
- `_clang`：与 `_gcc` 同模板，编译器换成 `clang/clang++`，condition 用 `anyOf(Linux, Darwin)` 同时覆盖 Linux 与 macOS
- `_clang-cl`：condition 为 `equals Windows`，且固化 `VCPKG_TARGET_TRIPLET: x64-windows`
- `_mingw-gcc`：condition 为 `equals Windows`，固化 `VCPKG_TARGET_TRIPLET: x64-mingw-dynamic`，使用 MSYS2 UCRT64 GCC
- `_mingw-clang`：condition 为 `equals Windows`，固化 `VCPKG_TARGET_TRIPLET: x64-mingw-clang-dynamic`，并通过 `VCPKG_OVERLAY_TRIPLETS` 指向仓库内 `triplets/`，使用 MSYS2 CLANG64 Clang/libc++
- `_gcc-conan` / `_clang-conan` / `_clang-cl-conan` / `_mingw-gcc-conan` / `_mingw-clang-conan`：把 `inherits` 中的 `_vcpkg` 换成 `_conan`，condition 与各自的 vcpkg 版一致

> `_gcc` 与 `_gcc-conan` 都仅在 Linux 主机可见 —— **不开放给 macOS**。macOS 上 `gcc`/`g++`
> 默认是 Apple Clang shim，跑出来其实是 clang，会让 preset 名与实际编译器对不上。Mac
> 用户应当用 `clang-*` preset。
...
| 编译器家族 | vcpkg leaf | Conan leaf | 单家族 leaf 合计 |
| --- | --- | --- | --- |
| GCC（仅 Linux） | `gcc-{debug,release,relwithdebinfo,minsizerel}` | `gcc-{...}-conan` | 8 |
| Clang（Linux + macOS） | `clang-{debug,release,relwithdebinfo,minsizerel}` | `clang-{...}-conan` | 8 |
| clang-cl（Windows） | `clang-cl-{debug,release,relwithdebinfo,minsizerel}` | `clang-cl-{...}-conan` | 8 |
| MinGW GCC（Windows） | `mingw-gcc-{debug,release,relwithdebinfo,minsizerel}` | `mingw-gcc-{...}-conan` | 8 |
| MinGW Clang（Windows） | `mingw-clang-{debug,release,relwithdebinfo,minsizerel}` | `mingw-clang-{...}-conan` | 8 |
| MSVC VS（Windows，多配置） | `msvc` | `msvc-conan` | 2 |
| MSVC Ninja-MC（Windows，多配置） | `ninja-mc-msvc` | `ninja-mc-msvc-conan` | 2 |
| **合计 configurePreset leaf** | | | **44** |

> buildPresets / testPresets 数都是 **56**：单配置家族每条 configurePreset 自动有 1
> 个对应的 build/test preset；MSVC / Ninja-MC 的两个多配置家族（共 4 个 configurePreset）
> 每个再展开成 4 个 build/test preset（每种 build_type 一个），所以多配置家族多出
> 12 个；4 + 12 = 16 个比 leaf 多出来的部分，正好让总数从 44 走到 56。

### 命名约定

- vcpkg 版：`{compiler}-{buildtype}`（例：`gcc-debug`）
- Conan 版：`{compiler}-{buildtype}-conan`（例：`gcc-debug-conan`）
- 多配置 family：仅 family 名（例：`msvc` / `ninja-mc-msvc` / `msvc-conan` / `ninja-mc-msvc-conan`），buildPreset 用 `{family}-{buildtype}` 选 configuration
...
MinGW（MSYS2 ucrt64 / clang64 环境）虽然在 Windows 上，但用 GNU 工具链 + GNU ABI。本仓库的
`_gcc` 在 Windows 上不可见，这正是 condition 想避免的"误用"。仓库为 MinGW 单独提供了
两组一等公民 preset：`mingw-gcc-*` 与 `mingw-clang-*`，门控为
`hostSystemName == Windows`。其中 `mingw-gcc-*` 固化为 `x64-mingw-dynamic`，
`mingw-clang-*` 固化为仓库 overlay triplet `x64-mingw-clang-dynamic`，开箱即用。详见
...
缺点：preset 数量多 —— 本仓库 5 个单配置编译器家族（gcc / clang / clang-cl / mingw-gcc /
mingw-clang）× 4 个 build types × 2 套依赖管理器（vcpkg / Conan）就有 40 个 leaf；再加上
3 个多配置 family（`msvc` / `ninja-mc-msvc` 各自的 vcpkg + Conan 版本）就到 44 个。
...
- `noTestsAction: error`：当 `ctest` 没找到任何测试时直接返回非零，避免"先 `ctest` 后
  `cmake --build`"这种使用顺序产生「Total Tests: 0 + exit 0 = 假绿」。早期版本里这里是
  `ignore`（理由是有些 demo-only 模块没有测试），后来改用模块自身按需 `mcpp_add_test`
  就够了 —— 整个 build tree 只要至少注册了一个测试，`error` 就不会误伤；而它能挡住的是
  真正危险的"以为绿了实际没跑"。
...
> 这一节。
>
> 关于 Clang 如何通过 target triple 切换 ABI、以及为什么 `mingw-clang` preset 需要
> chainload toolchain 的底层原理，详见
> [`cpp-compilation-abi-guide.md` §8-10](cpp-compilation-abi-guide.md#8-clang-的多目标能力与-llvm)。
...
> 注：MinGW 的 vcpkg triplet 已经在仓库 preset 里固化：`mingw-gcc-*` 使用
> `x64-mingw-dynamic`，`mingw-clang-*` 使用 `x64-mingw-clang-dynamic`，
> **不再需要 UserPresets 叠层**。
