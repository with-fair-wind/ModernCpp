...
> ABI 的底层概念（调用约定、名字修饰、C++ stdlib 差异等）详见
> [`cpp-compilation-abi-guide.md`](cpp-compilation-abi-guide.md)，本节聚焦于 vcpkg
> triplet 的实操层面。
...
| Windows MinGW GCC | `mingw-gcc-*` | preset 内固化为 `x64-mingw-dynamic` |
| Windows MinGW Clang | `mingw-clang-*` | preset 内固化为 overlay triplet `x64-mingw-clang-dynamic` |
...
固化 `x64-windows`，`mingw-gcc-*` 固化 `x64-mingw-dynamic`，`mingw-clang-*`
固化仓库 overlay triplet `x64-mingw-clang-dynamic`，按编译器选 preset 即可（详见
[§6](#6-mingw-专题)）。

### 自定义 triplet

vcpkg 允许在仓库里放自定义 triplet（`.cmake` 文件），通过
`VCPKG_OVERLAY_TRIPLETS` 启用。

本仓库的 `mingw-clang-*` preset 使用了自定义 triplet `x64-mingw-clang-dynamic`
（位于 `triplets/x64-mingw-clang-dynamic.cmake`），因为内置 `x64-mingw-dynamic`
会让 vcpkg 用 GCC + libstdc++ 编译包，而 MSYS2 clang64 的编译器使用 libc++，
两者 C++ stdlib ABI 不兼容。自定义 triplet 通过 `VCPKG_CHAINLOAD_TOOLCHAIN_FILE`
强制 vcpkg 内部也用 clang64 的 Clang，确保整条链路在同一 ABI 生态内闭合。

底层原理详见 [`cpp-compilation-abi-guide.md` §9-10](cpp-compilation-abi-guide.md#9-msys2-子系统与-abi-对应关系)。
...
- `mingw-gcc-*`：triplet 固化为内置 `x64-mingw-dynamic`，编译器用
  `x86_64-w64-mingw32-gcc` / `x86_64-w64-mingw32-g++`（MSYS2 ucrt64）
- `mingw-clang-*`：triplet 固化为自定义 `x64-mingw-clang-dynamic`（通过
  `VCPKG_OVERLAY_TRIPLETS` 指向 `triplets/`），编译器用
  `x86_64-w64-mingw32-clang` / `x86_64-w64-mingw32-clang++`（MSYS2 clang64）
...
切到 MinGW Clang：换到 MSYS2 CLANG64 shell（或确保 `clang64/bin` 在 PATH 前面），
再把 `mingw-gcc-` 换成 `mingw-clang-`。
