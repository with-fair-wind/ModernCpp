...
> 仓库实际 conanfile 没有 `[layout]` 段（顶部注释解释了原因：要让
> `--output-folder=X` 直接把 `conan_toolchain.cmake` 落在 `X/` 而不是
> `X/build/<BuildType>/generators/`，preset 路径才好引用）。下面的字段解释
> 仍然保留 `[layout]` 介绍，作为通用参考。
...
**一条独立的 Conan 通道**：`linux-gcc-conan` —— GCC 13 + `linux-gcc` host profile +
