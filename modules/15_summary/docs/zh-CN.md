...
- 列出 `<cmath>` 中常用函数表（附特殊说明）：

| 类别 | 函数 | 说明 |
|---|---|---|
| 三角函数 | `sin, cos, tan, asin, acos, atan, atan2` | `atan2(y,x)` 可区分 `y/x` 与 `-y/-x`。 |
| 双曲函数 | `sinh, cosh, tanh, asinh, acosh, atanh` | 无特殊说明 |
| 指数/对数函数 | `exp, exp2, expm1, log, log2, log1p, log10, logb` | `expm1` 即 `eˣ−1`（exp minus 1），`log1p` 即 `ln(x+1)`（ln 1 plus），`logb` 即 `floor(log2(x))`。 |
| 幂函数 | `pow, sqrt, cbrt, hypot` | `cbrt` 即 `∛x`，`hypot(x,y[,z])` 即斜边 `√(x²+y²(+z²))`。 |
| 舍入 | `ceil, floor, trunc, round, nearbyint, rint` | `trunc(2.1)==2`，`trunc(-2.1)==-2`；`round` 不考虑当前舍入模式但 `nearbyint` 会；`rint==nearbyint` 但当整数不可精确表示时额外报错。 |
| 除法 | `fmod, remainder, remquo, div` | 设 `quo=x/y`，`remainder` 为 `x-round(quo)*y`，`fmod` 为 `x-trunc(quo)*y`。`remquo(x,y,*quo)` 可用于周期函数。`div(x,y)->div_t` 对整数同时取商和余。 |
| 分解与组合 | `frexp, ldexp, scalbn, modf` | `frexp(x,*e)->m: x=m*2ᵉ, m∈[0.5,1)`；`ldexp(m,e): x=m*2ᵉ`（`scalbn` 类似但底不限于 2）；`modf(x,*intptr)->frac: x=int+frac`。 |
| 分类 | `fpclassify, isfinite, isinf, isnan, isnormal` | `normal`：规格化数。`fpclassify` 返回整数分类值。 |
| 比特操作 | `signbit, copysign, nextafter, nexttoward` | `signbit(x)->bool`；`copysign(x,y)` 把 `y` 的符号赋给 `x`；`nextafter(from,to)` 返回朝 `to` 方向的下一可表示数。`nexttoward` 有细微差异。 |
| 比较 | `isgreater, isless, isgreaterequal, islessequal, islessgreater, isunordered` | 无特殊说明 |
| 其他-1 | `erf, erfc, tgamma, lgamma` | `erf(x) = 2/√π ∫₀ˣ e⁻ᵗ² dt`，即正态分布 `N(0,0.5)` 在 `[-x,x]` 上的概率积分。`erfc(x) = 1-erf(x)`。`tgamma` 是 Γ 函数，`lgamma` 是 `ln|Γ(x)|`。 |
| 其他-2 | `fmin, fmax, abs, fdim, fma, nan` | `fdim(x,y) = max(0, x-y)`；`fma(x,y,z) = x*y+z`；`nan(const char* arg)` 把字符串转为 NaN 的特定二进制表示（如 `nan("1")` 是 `0x7ff0000000000001`）。 |

因编译器可自动合并除法与取模，`div` 现已不常用。

# 数学函数

- 注 1：这些函数通常对应专用指令，标准库或编译器优化会自动使用。
- 例如 `1.0f / sqrtf(x)` 在 Intel 上会优化为 `RSQRTSS`；
- 但性能有时反而会下降…

1. GNU 扩展提供 `sincos`，历史上在 x87 架构有专用指令 `fsincos` 而更快；但现在已比分开算更慢。
2. `ldexp(m, e)`（即 `x = m * 2ᵉ`）在使用 SIMD 指令时可能比 `m * (1 << e)` 更慢。

- 注 2.1：几乎所有函数都有带后缀 `f` 或 `l` 的变体，分别表示 `float` 或 `long double`。
- 如 `sinf` 用于 `float`，`sinl` 用于 `long double`。
- `scalbn` 略有特殊（虽不常用）：

# 数学函数

- 这些函数为与 C 的 `<math.h>` 兼容而存在。
- C 没有函数重载，只好用不同名称（无后缀表示 `double`）。
- C++ 实际上添加了重载：

用于 C++23 `<stdfloat>` 中的扩展浮点类型。

- 但需要显式类型控制时仍可用后缀版本。
- 注 2.2：部分函数有带前缀 `i`、`l` 或 `ll` 的变体，分别返回 `int`、`long` 或 `long long`。
- 例如 `logb` 返回 `floor(log2(x))`，`ilogb` 返回 `int(log2(x))`。
- 因返回类型不同，重载无法解决此类问题。

# 数学函数

- 一个完整示例：
- 但大多数函数并非拥有上述所有前后缀排列组合…

# 数学函数

- 注 2.3：建议完全限定函数名（即 `std::xxx`），以避免与 C 函数歧义。
- 例如：`abs(-3.1416)` 返回什么？（仅限 C++）
- 呃，取决于调用的是 C++ 函数还是 C 函数。
- C 没有 `abs(float)` 而引入了 `fabs`！
- 所以 `abs(-3.1416)` 可能是 `3` 也可能是 `3.1416`。
- 直接用 `std::abs` 即可！
- 注 3：还有一些常量，但 C++ 中应使用 `<limits>` 中的 `std::numeric_limits`。

# 数学函数

- 注 4：数学函数的错误处理通过浮点异常或 `errno`（由宏决定具体方式）。

浮点异常可用 `fetestexcept` 测试（此处不深入）。

# 数学函数

- 注 5：自 C++23 起，大部分这些函数变为 `constexpr`。

- 注 6：自 C++26 起，大部分函数增加显式 SIMD 重载。
- 因 C++26 引入 `<simd>`；但此处不涉及。

- 注 7：关于数值精度的一些特别说明…
- 先介绍最后一位单元（ULP, unit in the last place）。

# ULP

- ULP 即当前浮点数附近的间距。
- 即 `ULP(x) = nextafter(x, +inf) - x`（`x > 0`）；`ULP(x) = x - nextafter(x, -inf)`（`x < 0`）。
- 在 IEEE 754 下：
- 设浮点有 `p` 位尾数，`x` 的指数为 `e`；
- 对规格化数，ULP 为 `2^(e-p)`；
- 对非规格化数，ULP 为 `2^(e-p+1)`。
- 例：计算 `float x = 1.0f` 的 ULP。
- 32 位 `float` 有 23 位尾数；`1.0` 的指数为 `0`（因 `1.0 = 1.0 × 2⁰`）。
- 故 `ULP(1.0f) = 2⁻²³`。
- 借助 ULP 可描述有限精度浮点运算的误差。

Catch2 提供 `WithinULP(x, d)` 用于检查误差是否在 `d · ULP(x)` 内。

# ULP

- IEEE-754 保证部分运算误差在 0.5 ULP 内：
- 舍入：给定任意值 `x`，转为浮点后选最近的可表示值，即误差在 `0.5 * ULP(x)` 内。
- 加法：`(a ⊕ b) - (a + b) ∈ 0.5 ULP(a + b)`。
- 即给定 `float a + float b` 的计算结果（记为 `a ⊕ b`）与真实值 `a + b`（也称无限精度值），差距不超过 `0.5 ULP(a + b)`。
- 当然假设 `a, b` 本身已可精确表示。
- 类似地，减法、乘法与除法也在 `0.5 ULP` 内。
- 最重要的是，IEEE-754 要求 `sqrt` 与 `fma` 同样在 `0.5 ULP` 内。
- 故有精度要求时，优先用 `fma` 而非 `a ⊗ b ⊕ c`。

图形学中的极端误差案例可自行查阅。

# 数学函数

- 注 8：部分函数在 `<complex>` 中增加了复数计算版本。
- `std::complex<T>` 大致等于 `T real, imag` 的封装；此处不深入。

也可使用用户定义字面量。

`std::complex` 的方法，相当简单。

# 数学函数

- 最后，C++17 增加了许多特殊数学函数。
- 此处不深入；在你的任务需要时直接使用即可。

例如图形学常用的球谐函数（Spherical Harmonics），可用 `sph_legendre` 计算。

# 数学常数

- C++20 在 `<numbers>` 中添加了多种数学常数。
- 命名空间为 `std::numbers`。
- 去掉 `_v` 后表示 `double` 常数（如 `std::numbers::e`）。
- 若要 `float` 常数，需用如 `std::numbers::e_v<float>`。

# 附注

- 另一附注：C++26 增加了一些整数算术工具。
- 饱和运算（Saturation）：在 `<numeric>` 中；`saturating_xxx<T>(x, y) -> T` 对 `x` 和 `y` 做饱和计算，结果越界时钳位到可表示值。
- `xxx` 可为 `add`、`sub`、`mul`、`div`。
- `saturating_cast<T>(x) -> T`；对比之下 `(T)x` 会做模运算。例如 `(int8_t)-696` 是 `72`，而 `saturating_cast<int8_t>(-696)` 是 `-128`。
- 示例：
- 检查运算（Checked）：在 `<stdckdint.h>`（首先被 C23 接受）中；当 `x` 和 `y` 的计算结果越界时，`ckd_xxx(*result, x, y) -> bool` 不设置 `*result` 并返回 `false`。
- `xxx` 可为 `add`、`sub`、`mul`。
...
- C 只提供非常简单的 `rand` 函数获取伪随机整数。
- 且只能用 `srand` 设种子来操控。
- 但它有许多缺点…

1. 非线程安全，需加锁保护；
2. 许多数学公式需要特定分布；仅提供随机整数不够。
3. 为向后兼容，内置算法非常朴素，很可能生成低质量随机数。

- 即随机数可能很快出现循环。
- 为克服这些问题，C++11 引入 `<random>`。

实现通常使用 `/dev/urandom` 或 OS 随机库。但你也可在构造函数中用其他字符串（如 `/dev/random`）显式指定。

# 随机数

- 物理中有许多「真」随机过程。
- 例如放射性粒子（radioactive particle）是否衰变完全随机；我们只能用半衰期（half-life）统计描述其整体行为。
- 在计算机中，可利用反向偏置二极管（reverse-biased diode）的噪声。
- C++ 提供「真」随机数生成器 `std::random_device`。
- 通常从硬件设备获取随机数。
- 然而当真随机源不可用时，也可用软件实现的伪随机数生成器（PRNG）替代。
- 可用 `.entropy() -> double` 检查；
- PRNG 实现「会」返回 0，而其他返回正值。

# 随机数

- 但通常 `std::random_device` 频繁调用时性能会急剧下降（因须「冲刷噪声」）。
- 故通常只用它为 PRNG 生成种子。
- C++ 提供一系列不同算法的 PRNG：
...
- 这些 PRNG 生成均匀分布的随机整数；
- 但科学问题中，通常需要服从特定分布的随机数。
- C++ 还提供大量接受 PRNG 的分布类，将生成的数变换为服从相应分布。
- 此处不逐一详述；需要时查手册。
...
构造函数：应可拷贝；拷贝共享相同内部状态。

`op()`：生成 `[min, max]` 内的随机整数并前进到下一状态。`.discard(unsigned long long n)`：丢弃后续 `n` 个随机整数。虽朴素实现只是循环 `op()` 共 `n` 次，但某些引擎有更优算法。

PRNG 可做相等比较与输入/输出（I/O 格式由实现定义）。

# 随机数

- 构造函数与 `.seed()` 接受种子来（重新）设置状态。
- 有若干变体：

1. 接受单个整数，如前所示。
2. 不接受参数，等同于对变体 1 用默认整数。

- 但这够了吗？
- **不够**。
- 原因：`M` 位整数只有 `2^M` 种状态；但某些引擎有更多状态位（例如 `std::mt19937` 有 624 个 32 位整数）。
- 故需要一个接受种子范围的变体。于是：

3. 接受 `SeedSequence`，可生成多个整数。

- 此重载是模板，标准库已提供满足 `SeedSequence` 的类 `std::seed_seq`。

# 随机数

- 例如：

- 给它 8 个随机 32 位整数做种子；虽仍远少于 624，但应优于单个整数。

- 因此 `SeedSequence` 是如下映射：
- 接受 `M` 个随机位；
- 输出 `N` 个尽可能消除偏差的随机位，用于初始化 PRNG 状态。

# 随机数

- `std::seed_seq` 有：

1. 构造函数：由一系列 32 位整数的范围初始化；
2. `.generate(begin, end)`：用消除偏差的随机整数填充 `[begin, end)`。
3. 获取 `v` 的状态。

- 注：当 `M = N`（即输入状态空间与输出空间相同）时不保证双射。例如给定两个 32 位整数，无法得到全部 64 位整数值。
...
1. `std::linear_congruential_engine`：即 LCG，广泛用于 C `rand()` 实现；`x_{i+1} = a · x_i + c mod m`，`x_0` 为种子。

- 占空间最少（仅一个整数 `x_i`），速度中等，随机质量最差。

2. `std::mersenne_twister_engine`：梅森旋转算法。

- 占空间最多（`mt19937` 有 624 个整数），速度极快，随机质量高（周期为 `2^M - 1`，其中 `M` 为梅森素数；`mt19937` 中 `M = 19937`）。空间为 `n` 个 `UIntType` 整数。

3. `std::subtract_with_carry_engine`：借位减法算法。

- 占空间中等，速度中等，随机质量中等。

空间为 `r` 个 `UIntType` 整数加一个标志位（0 或 1）。

# 随机数

- 部分 PRNG 算法需要对已有 PRNG 做变换，故有三个适配器：

1. `std::discard_block_engine`：每 `P` 个随机数只保留前 `R` 个。
2. `std::independent_bits_engine`：生成最后 `W` 个数值位独立的随机数。

- 可创建更大范围的随机数，如用 32 位引擎生成 64 位数。
- 结果存于 `UIntType`，其位数不应少于 `W`。

3. `std::shuffle_order_engine`：对引擎生成的随机数做洗牌。

- 维护大小为 `K` 的表，用 `K` 个引擎随机数填充；然后随机从表中选出下一随机数，并用引擎新随机数替换对应项。

为它们选择好的参数是专家级任务，故 C++ 提供大量预定义别名：

若无特殊需求，直接用 `std::default_random_engine`（实现中通常为 `std::mt19937`）。

# 随机数

- 上述 PRNG 均为迭代式，即 `x_{i+1} = f(x_i)` 取下一随机数。
- 然而这严重限制了随机数在并行中的使用。
- 为克服此问题，一种方式是使用如 `thread_local` 的 PRNG…
- 但此方法仍有许多缺点：

1. 初始化 PRNG 通常开销大，但每个线程都须初始化；
2. 增加线程时可复现性弱。

- 新线程引入完全不同的序列，导致不同的数值结果。

3. 需要提供多个好的种子。

- 那如果能提供一种 `x_i = f(i)` 的 PRNG 呢？
- 为每个线程提供任务 ID 是多线程任务中非常常见的模式，因此这种 PRNG 使随机数获取变得简单。

# 随机数

- C++26 增加了基于计数器的 PRNG `std::philox_engine`。
- 此引擎也广泛用于 Pytorch 的不同设备上。
- 其周期为 `n * 2^(n * sizeof(UIntType))`。
- `philox_engine` 的状态包含：

1. 整数计数器；为表示长整数，使用 `std::array<UIntType, n>` 数组。
2. 半位宽的整数种子，即大小为 `n/2` 的数组。
3. 缓冲区 `b` 存储生成结果，位宽相同，即大小为 `n` 的数组。
4. 索引 `i` 用于分割缓冲区并正确获取下一随机数。

- 其 `operator() -> UIntType` 返回 `b[i]` 并递增 `i`。
- 当 `i` 到达 `n` 时，运行 philox 算法生成下一个 `b`，并将计数器增 1。

# 随机数

- 例如：

1. 构造函数：需要 `4/2 = 2` 个 32 位整数作种子；我们只提供一个，另一个置 0。
2. 计数器：初始化为全 0。
3. `operator()`：先生成四个 32 位整数，此处消费两个。

- 若消费 5 个整数，则第 5 个需：(1) 增加计数器；(2) 生成下一批四个 32 位整数。
- 现可提供 `4 · 2^(4×32)` 个随机数。
- 那如何便于并行任务？
- C++ 提供 `.set_counter()` 偏移计数器，因此你可以把数列分成任意块！
...
每个子任务占据以 `{atom_id, time_step}` 为首的序列，因此可自由消费 `4 · 2^(2×32)` 个随机数。

- 虽然其他 PRNG 也可用 `.discard` 达到类似效果，但其消耗 `O(log n)` 或 `O(n)`（`n` 为跳转距离）；`philox_engine` 以 `O(1)` 完成。

# 最后说明

1. 关于可复现性：

- PRNG 算法被严格规定，故只要种子相同，所有平台生成相同序列。
- 但分布算法**不**被规定，故即便 PRNG 相同，不同平台的分布也可能生成不同序列。
- 若要确保各处产生相同随机值，`<random>` 不够。

2. `<random>` 不关心密码安全（cryptographical security）。

- 密码安全意味着：即便知道 PRNG 算法和部分序列，仍无法在多项式复杂度内推断序列其余部分或猜出种子。
- 应使用如 OpenSSL 的安全随机序列。

# 最后说明

3. 部分未涉及的工具：

- `generate_canonical()`：给定 PRNG，生成 `[0, 1)` 内的随机浮点数，最多 `Bits`（但不超过 `sizeof(RealType)`）位随机性。
- 实现用 PRNG 生成随机位。
- 例如下面程序生成 10 位随机性的 `double`（即至多 `2^10` 种可能值）。

- 以及 `<algorithm>` 中与随机相关的算法：

1. `shuffle`：用 PRNG 洗牌随机访问范围。

- URBG 即均匀随机位生成器，在 C++ 中等同于 PRNG（因它们均匀生成整数）。

随机算法的细节同样不被严格规定，故跨平台不可复现。

# 随机数

2. `sample`：自 C++17 起，从 `[first, last)` 中随机选 `n` 个元素输出到 `out`，每个元素出现概率相等。

- 每个元素在输出范围中最多出现一次。

- 以上两者自 C++20 起也有 `std::ranges` 版本。

3. `generate_random(range, PRNG[, distribution])`：自 C++26 起，仅 `std::ranges` 版本。

- `range` 也可用两个参数 `begin, end`；
- 效果：用 PRNG 或分布填充范围。

问题：为什么不直接用 `std::generate`？

标准鼓励标准库实现在可能优化时为 PRNG 和分布添加成员 `.generate_random`。

# 随机数

- 原因：`stdr::generate_random` 允许定制化。
- 例如某些 PRNG 或分布可被向量化，但朴素的回退循环会抑制向量化。
- 这些 PRNG 或分布可提供成员函数 `.generate_random()`，`stdr::generate_random` 会调用它们。
...
- 我们讨论了算法与容器的实现，以及 C++20 ranges 的入门。

5. 生命周期（与类型安全）

- 生命周期与存储期、严格别名规则、切片问题、C++ 风格类型转换、variant 与 any。

# 课程大纲

6. 多文件编程

- 预处理器、翻译单元、ODR、命名空间与链接（及 `inline`）等基本原理，使我们能解释头文件与源文件的工作原理。
- 少许 xmake 及如何制作库。
- 少许 C++20 模块。

7. 错误处理

- 从 C 错误码出发，逐步引入 optional 与 expected、异常、断言与调试辅助（如 stacktrace）。
- 注意 C++ 也封装了错误码，在本讲（即第 16 讲）作业中讨论。
- 我们强调了异常安全并讨论了 copy-and-swap 惯用法。
- 少许 Catch2 单元测试。
...
- 字符串字面量与原始字符串，string 与 string_view，以及 `<charconv>`。
- Unicode 工作原理及 C++ 如何支持。
- format 与 print 函数，扩展到范围，如何特化自定义 formatter。
- 少许流的工作原理。
- 最后是正则表达式，但不讨论 `<regex>`。

9. ~ 11. 移动语义，共 2.5 讲。

- 第 9 讲介绍了为何需要移动语义、如何编写移动构造/赋值、五/零法则、moved-from 状态及一些简单的移动语义算法。

# 课程大纲

- 第 10 讲讨论了值类别（加 decltype）、引用限定符与 deducing this、复制消除（RVO、NRVO、隐式移动）以及不同类型的性能分析。
- 第 11 讲讨论了万能引用与完美转发。

11. ~ 12. 模板，共 1.5 讲。

- 第 11 讲教授模板基础，包括 constexpr（consteval、constinit）、特化与重载决议、一些微妙细节（this->、typename、template、嵌套特化），以及 C++20 concept。
- 第 12 讲深入更难的部分：
- NTTP 与模板模板参数、类型推导（及 CTAD）、友元模板与懒实例化、SFINAE。
- 变参模板与折叠表达式。
- 重要技术：CRTP 与类型擦除。
...
- 第 13 讲从低层到高层揭示工具：从 thread（及 jthread 与停止令牌处理）、同步原语（信号量、互斥与锁、条件变量、latch 与 barrier）到 future-promise 模型、packaged_task 与 async。
- 第 14 讲详细讨论了内存序与原子变量。

14. 协程，共 0.5 讲。

- 简要介绍了协程，包括如何在 C++ 中编写、对称转移及 `std::generator`。

15. 内存管理

- 涵盖了对象布局、operator new 与 delete、智能指针与分配器。

# 课程大纲

16. 期末

- 最后补充了文件系统、chrono 与数学函数，并对整个课程做了总结。
- 哇，这是一段漫长的旅程…
- 当我提到每个部分时，你一定能回忆起学到的大量知识！
- 我们涵盖了大部分重要主题，某些如 ADL 被故意省略。
- 最终视频时长为 46 小时 + 5.5 小时（本补充章节），实际太长（我预期 40 小时），因为我想覆盖每个细节。
- 这是我的问题……如果重做本课程，许多内容会被删减。
- 如前所述，我们的课程只涉及到 C++23…

# 未来的 C++

- 虽然 C++ 提供了强大的工具，但它仍在演化，每月都有大量提案提交。
- 特别是 C++26 将带来以下重要特性：
- 我们已提到许多，如占位符名 `_`、rcu 与 hazard pointer、变参 friends、`copyable_function` 与 `function_ref`、constexpr placement new、`indirect` 与 `polymorphic`、`inplace_vector`（在作业中提及）等。此外：
- 包索引（Pack indexing）：

# 反射（Reflection）

- 示例：通用 formatter 打印成员。`template for` 叫做展开语句（expansion statement），也在 C++26 引入。
...
- 通用且统一的并发框架。
- 在 `<execution>` 中，命名空间 `std::execution`。

# 还有更多…

- 标准库加固（Standard library hardening）：给定特殊编译器参数（如 `-fharden`），标准库实现应始终检查条件！
- 例如 `vector::operator[]`：

- 当索引越界时是 UB；但加固要求实现检查前置条件。
- 安全性方面非常好的特性！
- 注意：语言 UB 与库 UB 不同；大量语言 UB 不可检测，但大多数库 UB 可检测。

# 还有更多…

- 协程的任务类型（Task type）：

- 我们在协程部分实现了一个简单的 Task，标准库进一步发展并将其迁入 `std::execution`。

1. `stdexec::just(0)` 是一个可被 `co_await` 的 sender。
2. 其 `.await_resume` 返回参数 `0`。
3. `co_return 0` 由 `stdexec::task::promise_type::return_value` 处理。

- 并行 Range 算法：
- 我们说过并行算法没有 `std::ranges` 版本；
- 自 C++26 起，对随机访问范围增加了 ranges 版本。
...
- 未初始化变量的错误行为（erroneous behavior）。
- 错误行为是定义明确的新行为（因此编译器不能像 UB 那样做错误假设与激进优化），但本质上是不正确的。
- 这限制了「未定义」结果中的意外。
- 若确实需要未初始化，可使用属性：
...
- 嗯，我们「现代 C++ 基础」的课程到此结束了…
- 我真的非常感谢坚持学习本课程并走到这里的观众！
- 虽然本系列「超出了自己的生命周期」，但所有知识已传递到你的数据成员中，将继续随你一起冒险。

- 你在 C++ 中一定会学到越来越多…
- 请相信自己，砥砺前行！
- 能成为这段终身旅程的一部分，是我的荣幸。

- 当我有足够的空闲时间时，本频道还将继续制作更多关于 C++ 有趣且前沿的主题内容。
- 真心感谢你，下个视频见 ☺。
